...
The proposed architecture shows that a compact neural discretizer can be effectively combined with a tabular reinforcement learning controller in a simplified autonomous driving task. The SOM reduces continuous lidar readings to a finite number of states, making Q-learning feasible and interpretable. The curriculum learning procedure and reward shaping terms were necessary to guide the agent toward successful behavior, especially because reaching the finish line is a sparse event.

The experiments explored multiple configurations. The most relevant design factors were the number of lidar rays, the steering range and the size of the SOM grid. Richer perception, especially with five lidar rays, provides additional information about obstacles in front of the vehicle and about the symmetry of free space around the car. Larger SOM grids provide more representational capacity, although they also increase the number of state-action pairs that must be learned.
...
\begin{itemize}
    ...
    \item improve reward shaping for overtaking or bypassing central obstacles;
    ...
    \item improve the physical realism of the simulator;
    \item generate larger benchmark track sets procedurally;
    \item transfer the approach to a more realistic simulator or to a small robotic platform.
\end{itemize}

Overall, the project demonstrates a full learning-control workflow for autonomous navigation in a controlled environment. Its main value is not only the final policy, but also the modular framework that makes it possible to draw new tracks, retrain state representations, compare reinforcement learning configurations and evaluate the resulting behavior with reproducible metrics.
