...
The project was implemented in Python using modular components for track creation, environment simulation, SOM training, reinforcement learning training, inference and quantitative evaluation. The final workflow supports both interactive visual inspection through Pygame and headless evaluation over a collection of test tracks. An example of the system in action is shown in \Cref{fig:example}.
...
\begin{figure}[ht]
    ...
    \caption{An example of the system in action. The car (yellow rectangle) is navigating a track with borders (grey lines) and obstacles (red circles). The lidar sensors are represented as blue rays, and the target finish line is shown in yellow.}
    \label{fig:example}
\end{figure}
